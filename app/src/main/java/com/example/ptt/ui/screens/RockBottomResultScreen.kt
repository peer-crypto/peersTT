...
fun RockBottomResultScreen(
    ...
    Column(
        ...
        if (vm.calcGasL != null && vm.calcBar != null) {
            Text("Gas until first switch", style = MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(4.dp))
            Text("${vm.calcGasL} L", style = MaterialTheme.typography.headlineMedium)

            Spacer(Modifier.height(16.dp))

            Text("Required cylinder pressure", style = MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(4.dp))
            Text("${vm.calcBar} bar", style = MaterialTheme.typography.headlineMedium)

            // Platzhalter: Hier später „Herleitung/Segments“ einblenden
            Spacer(Modifier.height(24.dp))
            Text("Details will follow (derivation & segments)", style = MaterialTheme.typography.bodySmall)
        } else {
            Text("No result available. Please go back and calculate.", style = MaterialTheme.typography.bodyMedium)
        }
    }
}
