...
object Routes {
    ...
    const val ROCKBOTTOM = "rockbottom" // ⬅️ neu
}
    ...
    NavHost(navController = nav, startDestination = Routes.HOME) {
        composable(Routes.HOME) {
            HomeScreen(
                ...
                onRockBottom = { nav.navigate(Routes.ROCKBOTTOM) } // ⬅️ neu
            )
        }
        composable(Routes.DEEPSTOP) { DeepstopScreen(onBack = { nav.popBackStack() }) }
        composable(Routes.ROCKBOTTOM) { RockBottomScreen(onBack = { nav.popBackStack() }) } // ⬅️ neu
    }
}
